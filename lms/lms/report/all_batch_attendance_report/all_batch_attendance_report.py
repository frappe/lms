...
#             COUNT(DISTINCT CASE WHEN a.status = 'Present' THEN a.member END) as present_count,
#             COUNT(DISTINCT CASE WHEN a.status = 'Absent' THEN a.member END) as absent_count,
#             GROUP_CONCAT(DISTINCT be.member_name SEPARATOR ', ') as invited_users,
#             GROUP_CONCAT(DISTINCT CASE WHEN a.status = 'Present' THEN be.member_name END SEPARATOR ', ') as attended_users
#         FROM `tabLMS Batch` b
#         LEFT JOIN `tabLMS Batch Enrollment` be ON be.batch = b.name
#         LEFT JOIN `tabLMS Attendance` a ON a.batch = b.name AND a.member = be.member
#         WHERE {conditions}
#         GROUP BY b.name
#     """
