...
const STORAGE_ENTITIES: Record<string, string> = {
	...Object.fromEntries(
		Object.entries(HTML_ESCAPE_MAP as Record<string, string>).map(
			([char, entity]) => [entity.toLowerCase(), char]
		)
	),
	'&#34;': '"',
	'&#x22;': '"',
...
const STORAGE_ENTITY_RE = new RegExp(
	`(?:${Object.keys(STORAGE_ENTITIES).join('|')})`,
	'gi'
)
...
function decodeFromStorage(stored: string): string {
	...
	return stored.replace(
		STORAGE_ENTITY_RE,
		(entity) => STORAGE_ENTITIES[entity.toLowerCase()] ?? entity
	)
}
...
export class CodeBox {
	...
	constructor({ data, api, config, readOnly }) {
		...
		this.data = {
			code:
				data.format === 'text'
					? decodeFromStorage(rawCode)
					: legacyHTMLToPlainText(rawCode),
			format: 'text',
			language:
	...
	_createLanguageSelectElement() {
		...
		this.selectInput.value =
			(COMMON_LANGUAGES as Record<string, string>)[this.data.language] ??
			this.data.language
		this.api.listeners.on(
			this.selectInput,
			'click',
	...
	_highlightCodeArea(event?) {
		...
		const result = hljs.highlight(this.data.code, {
			language: resolveCodeBoxLanguage(this.data.language),
		})
	...
	_handleCodeAreaPaste(event) {
		// Insert the plain-text payload only, with no outside styling.
