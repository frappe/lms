...
// Does NOT scroll: MobileLayout's <main id="scrollContainer"> is the app's
// one scroller; a second one here breaks skip-to-content and scroll-to-top.
// FormShell keeps its own scroller (`fixed inset-0`, outside this container).
//
// Exactly one h1 renders whether or not the header shows: root tabs take
// their identity from the bottom bar, but heading rotors still need a name.
withDefaults(
	defineProps<{
		title: string
		showBack?: boolean
