...
// footer holds, so the footer yields that edge while a selection lasts
// instead of being covered; it returns once the selection clears.
...
// The `role="status"` span outlives the v-if chain, so a loading message has
// somewhere to land; the skeleton itself is `aria-hidden`. It watches the
// whole flag, not `rows.length`, because a filter keeps old rows visible.
const loadedMessage = () => {
