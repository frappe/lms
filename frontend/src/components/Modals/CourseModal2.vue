...
const processFolder = async (e) => {
  ...
  // Function to get or create a nested folder structure
  const getOrCreateFolder = (structure, pathParts) => {
    return pathParts.reduce((currentFolder, part) => {
      // If the part (folder) does not exist, create it
      if (!currentFolder[part]) {
        currentFolder[part] = {}
      }
      return currentFolder[part]
    }, structure)
  }
  ...
  for (const file of Array.from(files.value)) {
    ...
    // Get the folder path, creating folders as needed
    const parentFolder = getOrCreateFolder(folderStructure.value, pathParts)

    // Upload the file and store the reference in the structure
    await uploadFile(file, parentFolder, fileName)
  }

  console.log("Folder structure:", folderStructure.value)
}
...
const uploadFolder = (name, parent="Home") => {
...
const uploadFile = (file, parentFolder, fileName) => {
	uploader.upload(file, {
		private: false,
		folder: "Home",
	}).then((data) => {
		parentFolder[fileName] = data
	})
}
