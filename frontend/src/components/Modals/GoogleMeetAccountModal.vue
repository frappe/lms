<template>
	<Dialog
		...
		:options="{
			title:
				accountID === 'new' ? __('New Google Meet Account') : __('Edit Google Meet Account'),
			size: 'xl',
			actions: [
				{
					label: __('Save'),
	...
	googleMeetAccounts.value?.insert.submit(
		...
		},
		{
			...
			onError(err) {
				close()
				toast.error(
					cleanError(err.messages[0]) || __('Error creating Google Meet Account')
				)
			},
		}
	)
	...
	googleMeetAccounts.value?.setValue.submit(
		...
		},
		{
			...
			onError(err: any) {
				close()
				toast.error(
					cleanError(err.messages[0]) || __('Error updating Google Meet Account')
				)
			},
		}
	)
