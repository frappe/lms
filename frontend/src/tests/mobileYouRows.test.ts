/**
 * The You page, as data. Pins the things a merge could quietly undo: no
...
// What stores/mobileNavLinks holds once settled for a moderator. No Settings
// link, since the phone has no settings page for one to point at.
const SIDEBAR = [
	link('Home', 'Home', 'Home'),
...
// Every fixture above names a real route, so the default answer is yes. A
// test that cares about the other shapes passes its own.
...
// The profile card is no longer a row: MobileYou draws it in its own template
// as a 96px circle over the name, covered by mobileYou.test.ts instead.
...
describe('destinations', () => {
	...
	it('keeps an unrecognised destination rather than dropping it', () => {
		...
		// falls through to MORE rather than a heading of its own; without that
		// fallback, a new sidebar link would silently become unreachable on a phone.
		const rows = build({
			sidebarLinks: [...SIDEBAR, link('Contact Us', 'Mail', 'ContactUs')],
		})
